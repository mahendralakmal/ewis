                            ...
                            @foreach ($products as $product)
                                @if($product->remove !=1)
                                    <form action="/client-profile/add-to-bucket" method="POST" class="side-by-side">
                                        {!! csrf_field() !!}
                                        <div class="row">
                                            <div class="col-md-1 text-center td-b" style="border: 1px solid #dddddd; padding:5px; height:45px;"><input type="hidden" id="id" name="id" value="{{ $product->id }}"><a href=""> {{$product->product->part_no}}</a></div>
